...
- `KB_LANGUAGE` — мова всього, що генерує AI (назва, виписка, ключові ідеї, гак, кут):
  `uk` (за замовчуванням), `en` або `auto` (мова самого контенту; мітки при цьому лишаються українськими)
- `KB_TAGS` — свій словник тегів через кому, напр. `"дослідження, найм, конкуренти"`
  (за замовчуванням: контент-ідея, продукт/курс, делівері, продажі, лідген)
...
pytest
```
Тести мережу не чіпають — ганяються локально й на сервері перед рестартом.
